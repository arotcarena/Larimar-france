...
class ProductRepositoryTest extends KernelTestCase
{
    ...
    //enQSearch
    public function testQSearchDontSearchIntoEnFields()
    {
        $products = $this->productRepository->qSearch('my obj');
        $this->assertCount(0, $products);

        $products = $this->productRepository->qSearch('ategory 1');
        $this->assertCount(0, $products);
    }
    public function testEnQSearchDontSearchIntoFrFields()
    {
        $products = $this->productRepository->enQSearch('on obj');
        $this->assertCount(0, $products);

        $products = $this->productRepository->enQSearch('atégorie 1');
        $this->assertCount(0, $products);
    }
    public function testEnQSearchWorks()
    {
        $products = $this->productRepository->enQSearch('y obj');
        $this->assertCount(1, $products);
        $this->assertEquals('my object', $products[0]->getEnDesignation());
    }
    public function testEnCountQSearch()
    {
        $count = $this->productRepository->enCountQSearch('y obj');
        $this->assertEquals(1, $count);
    }
    ...
    public function testFilterQWithLangParamEn()
    {
        ...
        $this->assertCount(1, $pagination->getItems());
        $this->assertEquals('my-object', $pagination->getItems()[0]->getEnSlug());
    }
    ...
    public function testFilterSelectOnlyProductsInStock()
    {
        $this->loadFixtures([ProductWithOrWithoutStockTestFixtures::class]);
        /** @var PaginationInterface */
        $pagination = $this->productRepository->filter((new SearchParams)->setQ('stock test'));
        $this->assertCount(1, $pagination->getItems());
        $this->assertTrue($pagination->getItems()[0]->getStock() > 0);
    }
