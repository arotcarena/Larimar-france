...
class ProductToArrayConvertorTest extends KernelTestCase
{
    ...
    public function testContainsCorrectKeysWhenConvertAll()
    {
        ...
        $returnProduct = $this->productConvertor->convert($products, 'en')[0];
    ...
    {
        $products = $this->productRepository->findAll();
        $data = $this->productConvertor->convert($products, 'en');
    ...
    {
        $product = $this->productRepository->findOneBy([]);
        $url = $this->urlGenerator->generate('en_product_show_withCategoryAndSubCategory', [
    ...
    public function testCorrectUrlIsSetWhenProductHaveCategoryButNoSubCategory()
    {
        ...
        $url = $this->urlGenerator->generate('en_product_show_withCategory', [
            'slug' => $product->getEnSlug(),
            'publicRef' => $product->getPublicRef(),
            'categorySlug' => $product->getCategory()->getEnSlug()
        ]);
    ...
    public function testCorrectUrlIsSetWhenProductHaveNoCategoryAndSubCategory()
    {
        ...
        $url = $this->urlGenerator->generate('en_product_show', [
            'slug' => $product->getEnSlug(),
            'publicRef' => $product->getPublicRef()
        ]);
