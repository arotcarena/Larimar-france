...
class AdminSubCategoryControllerTest extends AdminFunctionalTest
{
    ...
    public function testCreateInvalidSubCategoryWithExistantEnSlugAndSameParentCategory()
    {
        $this->loginAdmin();
        $category = $this->findEntity(CategoryRepository::class, ['slug' => 'categorie-1']); // category avec 3 subCategory
        $existantSubCategory = $category->getSubCategories()->get(0);
        $this->submitForm(
            ...
            'Ajouter', 
            $this->createValidData([
                'enSlug' => $existantSubCategory->getEnSlug(),
                'parentCategory' => (string)($category->getId())
            ])
        );
    ...
    public function testCreateValidCategoryCorrectPersist()
    {
        ...
        $this->submitForm(
            ...
            $this->createValidData([
                ...
                'enName' => 'sub category test',
                'slug' => 'une-sous-categorie-de-test',
                'enSlug' => 'sub-category-test',
                'listPosition' => '25',
                'parentCategory' => $category->getId()
            ])
        );
        ...
        $this->assertEquals('sub category test', $persistedSubCategory->getEnName());
        $this->assertEquals('sub-category-test', $persistedSubCategory->getEnSlug());
    ...
    public function testUpdateCorrectChanges()
    {
        ...
        $this->submitForm(
            ...
            [
                ...
                'sub_category[enName]' => 'updated subcategory en name',
                'sub_category[slug]' => 'updated-subcategory',
                'sub_category[enSlug]' => 'updated-subcategory-en-slug',
        ...
        $this->assertEquals('updated subcategory en name', $updatedSubCategory->getEnName());
        $this->assertEquals('updated-subcategory', $updatedSubCategory->getSlug());
        $this->assertEquals('updated-subcategory-en-slug', $updatedSubCategory->getEnSlug());
    ...
    public function testUpdateInvalidSubCategoryWithExistantEnSlugAndSameParentCategory()
    {
        $this->loginAdmin();
        //les parentCategory des subCategory ont toujours au moins 2 subCategories
        $oldSubCategory = $this->findEntity(SubCategoryRepository::class); 
        ...
        foreach($parentCategory->getSubCategories() as $subCategory)
        {
            if($subCategory->getId() !== $oldSubCategory->getId())
            {
                $existantSlug = $subCategory->getEnSlug();
            }
        }
        $this->submitForm(
            'admin_subCategory_update', 
            'Modifier', 
            [
                'sub_category[enSlug]' => $existantSlug
            ],
            ['id' => $oldSubCategory->getId()]
        );
        $this->assertSelectorTextContains('.enSlug-group .admin-form-error', 'existe déjà');
    ...
    public function testShowContainsCorrectSections()
    {
        ...
        $this->assertSelectorTextContains('.enName', $subCategory->getEnName());
        $this->assertSelectorTextContains('.slug', $subCategory->getSlug());
        $this->assertSelectorTextContains('.enSlug', $subCategory->getEnSlug());
    ...
    private function createValidData(array $options = []):array
    {
        ...
        $subCategoryData = [
            ...
            'sub_category[enName]' => 'new subcategory',
            'sub_category[slug]' => 'nouvelle-sous-categorie',
            'sub_category[enSlug]' => 'new-sub-category',
