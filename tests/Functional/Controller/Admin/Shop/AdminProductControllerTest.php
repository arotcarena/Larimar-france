...
class AdminProductControllerTest extends AdminFunctionalTest
{
    ...
    public function testCreateInvalidProductWithExistingSlugAndSameCategoryAndSubCategory()
    {
        ...
        $this->assertSelectorTextContains('.slug-group .admin-form-error', 'existe déjà');
    }
    public function testCreateInvalidProductWithExistingEnSlugAndSameCategoryAndSubCategory()
    {
        $this->loadFixtures([ProductWithOrWithoutCategoryTestFixtures::class, UserTestFixtures::class]);
        $product = $this->findEntity(ProductRepository::class, ['slug' => 'product-with-category-and-subcategory']);
        $this->loginAdmin();
        ...
        $this->assertSelectorTextContains('.enSlug-group .admin-form-error', 'existe déjà');
    }
    public function testCreateValidProductCorrectPersist()
    {
        ...
        $this->assertEquals('test product', $persistedProduct->getEnDesignation());
        $this->assertEquals('un-produit-de-test', $persistedProduct->getSlug());
        $this->assertEquals('description du produit de test', $persistedProduct->getDescription());
    ...
    public function testUpdateCorrectChanges()
    {
        ...
        $this->submitForm(
            ...
            [
                ...
                'product[enDesignation]' => 'en designation for updated product',
                'product[slug]' => 'updated-product',
                'product[enSlug]' => 'updated-product-en-slug',
                'product[description]' => 'description for updated product',
                'product[enDescription]' => 'en description for updated product',
        ...
        $this->assertEquals('en designation for updated product', $updatedProduct->getEnDesignation());
        $this->assertEquals('updated-product', $updatedProduct->getSlug());
        $this->assertEquals('updated-product-en-slug', $updatedProduct->getEnSlug());
        $this->assertEquals('description for updated product', $updatedProduct->getDescription());
        $this->assertEquals('en description for updated product', $updatedProduct->getEnDescription());
        $this->assertStringContainsString(
            'lessthan10k', 
    ...
    public function testUpdateInvalidExistingEnSlugWithSameCategoryAndSubCategory()
    {
        $this->loadFixtures([ProductWithOrWithoutCategoryTestFixtures::class, UserTestFixtures::class]);
        $productToCopySlug = $this->findEntity(ProductRepository::class, ['slug' => 'product-with-category-and-subcategory']);
        $oldProduct = $this->findEntity(ProductRepository::class, ['slug' => 'product-with-category']); // on spécifie le product qu'on veut pour être sur de pas tomber sur le même
        ...
        $this->submitForm(
            ...
            'Modifier', 
            [
                'product[enSlug]' => $productToCopySlug->getEnSlug(),
                'product[category]' => $productToCopySlug->getCategory()->getId(),
                'product[subCategory]' => $productToCopySlug->getSubCategory()->getId(),
            ],
            ['id' => $oldProduct->getId()]
    ...
    public function testShowContainsCorrectSections()
    {
        ...
        $this->assertSelectorTextContains('.enDesignation', $product->getEnDesignation());
        $this->assertSelectorTextContains('.slug', $product->getSlug());
        $this->assertSelectorTextContains('.enSlug', $product->getEnSlug());
        $this->assertSelectorTextContains('.description', $product->getDescription());
        $this->assertSelectorTextContains('.enDescription', $product->getEnDescription());
    ...
    private function createValidData(array $options = []):array
    {
        $productData = [
            ...
            'product[enDesignation]' => 'new product',
            'product[slug]' => 'nouveau-produit',
            'product[enSlug]' => 'new-product',
            'product[price]' => '10',
            'product[stock]' => '1',
