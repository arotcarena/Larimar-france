 ...
 * @group FunctionalApi
 */
class ApiAddressControllerTest extends FrFunctionalTest
{
    use LoginUserTrait;
    ...
    public function testNotLoggedUserCannotAccess()
    {
        ...
        //create
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_create'), [], [], [], json_encode($this->createValidAddressData()));
        $this->assertResponseRedirects($this->urlGenerator->generate('fr_security_login'));
        //update
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_update', ['id' => $id]), [], [], [], json_encode($this->createValidAddressData()));
        $this->assertResponseRedirects($this->urlGenerator->generate('fr_security_login'));
        //delete
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_delete'), [], [], [], json_encode($id));
        $this->assertResponseRedirects($this->urlGenerator->generate('fr_security_login'));
    }
    ...
    public function testIndexReturnAddressesWithCorrectKeys()
    {
        ...
        $this->client->request('GET', $this->urlGenerator->generate('fr_api_address_index'));
        $this->assertEquals(
            ['id', 'civility', 'firstName', 'lastName', 'lineOne', 'lineTwo', 'postcode', 'city', 'country', 'enCountry', 'iso', 'continents'],
    ...
    public function testCreateWithInvalidDataContainingIncorrectKeys()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_create'), [], [], [], json_encode([
    ...
    public function testCreateMoreThanMaxAddresses()
    {
        $this->loadFixtures([UserWithMaxAddressesTestFixtures::class]);
        $user = $this->findEntity(UserRepository::class, ['email' => 'user_with_max_addresses@gmail.com']);   //user avec MAX d'adresses enregistrées
    ...
    public function testCreateExactlyMaxAddresses()
    {
        $this->loadFixtures([UserWithMaxAddressesTestFixtures::class]);
        $user = $this->findEntity(UserRepository::class, ['email' => 'user_with_max_less_one_addresses@gmail.com']);   //user avec MAX - 1 d'adresses enregistrées
        
        $this->loginUser($user);
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_create'), [], [], [], json_encode($this->createValidAddressData()));
        $this->assertResponseIsSuccessful();
        //on vérifie que l'adresse a bien été persistée grace à son id qui est retourné par la function
        $id = json_decode($this->client->getResponse()->getContent());
        $address = $this->findEntity(AddressRepository::class, ['id' => $id]);
    ...
    public function testUpdateWithInexistantIdParam()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_update', ['id' => '123456789456']), [], [], [], json_encode([]));
        $this->assertResponseStatusCodeSame(Response::HTTP_INTERNAL_SERVER_ERROR);
        $this->assertNotNull(
            json_decode($this->client->getResponse()->getContent())->errors
        );
    ...
    public function testUpdateWithInvalidDataContainingIncorrectKeys()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_update', ['id' => $id]), [], [], [], json_encode([
            'invalidKey' => 'invalidData',
            'lineOne' => 'rue de la jungle',
            'city' => 'Issy Les Moulineaux'
        ]));
    ...
    {
        $this->loginUser();
        $this->client->request('POST', $this->urlGenerator->generate('fr_api_address_delete'), [], [], [], json_encode(123456789456));
        $this->assertResponseStatusCodeSame(Response::HTTP_INTERNAL_SERVER_ERROR);
        $this->assertNotNull(
            json_decode($this->client->getResponse()->getContent())->errors
        );
