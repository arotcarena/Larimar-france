 ...
 * @group FunctionalApi
 */
class ApiCartControllerTest extends FrFunctionalTest
{
    use FixturesTrait;
    ...
    public function testAddProductCountCart()
    {
        ...
        //il faut d'abord remove car on peut pas ajouter deux fois le même produit dans larimar
        $this->callRemove($product->getId());
        $this->callAdd($product->getId(), 2);
        $this->assertResponseIsSuccessful();
    ...
    public function testAddDifferentProducts()
    {
        ...
        $this->callAdd($products[0]->getId(), 1);
        $this->callAdd($products[1]->getId(), 1);
    ...
    public function testAddQuantityResultsOverStock()
    {
        ...
        $this->assertResponseStatusCodeSame(Response::HTTP_INTERNAL_SERVER_ERROR);
        $this->assertEquals(
            TextConfig::ERROR_ALREADY_IN_CART,
            json_decode($this->client->getResponse()->getContent())->errors
        );
    ...
    public function testGetFullCartCountDatabaseQueries()
    {
        ...
        $this->client->enableProfiler();
        $this->client->request('GET', $this->urlGenerator->generate('fr_api_cart_getFullCart'));
    ...
    public function testCount()
    {
        ...
        $this->client->request('GET', $this->urlGenerator->generate('fr_api_cart_count'));
    ...
    {
        $this->client->request('GET', $this->urlGenerator->generate('fr_api_cart_less', [
            'productId' => $productId,
            'quantity' => $quantity
        ])); 
    }
    ...
    {
        $this->client->request('GET', $this->urlGenerator->generate('fr_api_cart_remove', [
            'productId' => $productId
        ]));
    }
    private function callAdd($productId, $quantity)
    {
        $this->client->request('GET', $this->urlGenerator->generate('fr_api_cart_add', [
            'productId' => $productId,
            'quantity' => $quantity
        ]));
    }
    ...
    {
        $this->client->request('GET', $this->urlGenerator->generate('fr_api_cart_getLightCart'));
        return json_decode($this->client->getResponse()->getContent());
    }
