...
class ApiAddressControllerTest extends FunctionalTest
{
    ...
    public function testNotLoggedUserCannotAccess()
    {
        ...
        //create
        $this->client->request('POST', $this->urlGenerator->generate('en_api_address_create'), [], [], [], json_encode($this->createValidAddressData()));
        $this->assertResponseRedirects($this->urlGenerator->generate('en_security_login'));
        //update
        $this->client->request('POST', $this->urlGenerator->generate('en_api_address_update', ['id' => $id]), [], [], [], json_encode($this->createValidAddressData()));
        $this->assertResponseRedirects($this->urlGenerator->generate('en_security_login'));
        //delete
        $this->client->request('POST', $this->urlGenerator->generate('en_api_address_delete'), [], [], [], json_encode($id));
        $this->assertResponseRedirects($this->urlGenerator->generate('en_security_login'));
    }
    ...
    public function testIndexReturnAddressesWithCorrectKeys()
    {
        ...
        $this->client->request('GET', $this->urlGenerator->generate('en_api_address_index'));
        $this->assertEquals(
    ...
    public function testCreateWithInvalidDataContainingIncorrectKeys()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('en_api_address_create'), [], [], [], json_encode([
    ...
    public function testUpdateWithInexistantIdParam()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('en_api_address_update', ['id' => '123456789456']), [], [], [], json_encode([]));
        $this->assertResponseStatusCodeSame(Response::HTTP_INTERNAL_SERVER_ERROR);
        $this->assertNotNull(
            json_decode($this->client->getResponse()->getContent())->errors
        );
    ...
    public function testUpdateWithInvalidDataContainingIncorrectKeys()
    {
        ...
        $this->client->request('POST', $this->urlGenerator->generate('en_api_address_update', ['id' => $id]), [], [], [], json_encode([
            'invalidKey' => 'invalidData',
            'lineOne' => 'rue de la jungle',
            'city' => 'Issy Les Moulineaux'
        ]));
    ...
    {
        $this->loginUser();
        $this->client->request('POST', $this->urlGenerator->generate('en_api_address_delete'), [], [], [], json_encode(123456789456));
        $this->assertResponseStatusCodeSame(Response::HTTP_INTERNAL_SERVER_ERROR);
        $this->assertNotNull(
            json_decode($this->client->getResponse()->getContent())->errors
        );
    ...
    {
        return [
            'civility' => EnTextConfig::CIVILITY_F,
            'firstName' => 'Justine',
            'lastName' => 'Trudo',
