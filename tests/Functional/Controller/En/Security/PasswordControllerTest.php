...
class PasswordControllerTest extends FunctionalTest
{
    ...
    {
        $this->client->request('GET', $this->urlGenerator->generate('en_security_askResetPassword'));
    ...
    public function testCorrectEmailRedirectToLogin()
    {
        ...
        $this->assertSelectorTextContains('.alert.alert-success', EnTextConfig::ALERT_RESET_PASSWORD, 'le flash de succès n\'est pas présent');
    }
    ...
    {
        $this->client->request('GET', $this->urlGenerator->generate('en_security_resetPassword'), [
    ...
    public function testNewPasswordWithValidPasswordRedirectToLogin()
    {
        ...
        $this->assertResponseRedirects($this->urlGenerator->generate('en_security_login'));
        $this->client->followRedirect();
        $this->assertSelectorTextContains('.alert.alert-success', EnTextConfig::ALERT_RESET_PASSWORD_SUCCESS);
    }
    ...
    private function loginUserAndTryChangePassword(string $oldPassword, string $password, string $passwordConfirm)
    {
        ...
        $crawler = $this->client->request('GET', $this->urlGenerator->generate('en_security_changePassword'));
    ...
    {
        $crawler = $this->client->request('GET', $this->urlGenerator->generate('en_security_askResetPassword'));
        $form = $crawler->selectButton('Submit')->form([
            'email' => $email
        ]);
        $this->client->submit($form);
    ...
    private function submitPassword(string $password, string $passwordConfirm, ?User $user = null)
    {
        ...
        $crawler = $this->client->request('GET', $this->urlGenerator->generate('en_security_resetPassword'), [
            'token' => $user->getId() . '==' . $user->getResetPasswordToken()
        ]);
        $form = $crawler->selectButton('Submit')->form([
