...
class ProductWithOrWithoutCategoryTestFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $product->setPublicRef(str_shuffle('aAzZeErRtT0123456789'))
                ...
                ->setEnDesignation('product with category and subcategory en')
                ->setSlug(
                    'product-with-category-and-subcategory'
                )
                ->setEnSlug(
                    'product-with-category-and-subcategory-en'
                )
        ...
        $product->setPublicRef(str_shuffle('aAzZeErRtT0123456789'))
                ...
                ->setEnDesignation('product with category en')
                ->setSlug(
                    'product-with-category'
                )
                ->setEnSlug(
                    'product-with-category-en'
                )
                ->setPrice(random_int(2000, 20000))
                ->setStock(1)
                ->setCategory($category)
        ...
        $product->setPublicRef(str_shuffle('aAzZeErRtT0123456789'))
                ...
                ->setEnDesignation('product without category or subcategory en')
                ->setSlug(
                    'product-without-category-or-subcategory'
                )
                ->setEnSlug(
                    'product-without-category-or-subcategory-en'
                )
                ->setPrice(random_int(2000, 20000))
                ->setStock(1)
                ->setCreatedAt(new DateTimeImmutable())
