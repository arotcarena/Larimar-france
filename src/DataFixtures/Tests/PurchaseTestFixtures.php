...
class PurchaseTestFixtures extends Fixture implements DependentFixtureInterface
{
    ...
    public function load(ObjectManager $manager)
    {
        ...
        $manager->persist($purchase);
         ...
         //purchase-with-product-having-category-and-subcategory
        /** @var User */
        $user = $this->faker->randomElement($users);
        $user->setCivility($this->faker->randomElement([TextConfig::CIVILITY_F, TextConfig::CIVILITY_M]))
            ->setFirstName($this->faker->firstName())
        ...
        $product = $this->productRepository->findOneBySlug('product-with-category-and-subcategory');

        $purchase = (new Purchase)
                        ->setRef('purchase-with-product-having-category-and-subcategory')
                        ->setUser($user)
                        ->setDeliveryDetail($postalDetail)
                        ->setInvoiceDetail($postalDetail)
                        ->setStatus(SiteConfig::STATUS_PENDING)
    ...
    {
        return [UserTestFixtures::class, ProductTestFixtures::class, ProductWithOrWithoutStockTestFixtures::class, ProductWithOrWithoutCategoryTestFixtures::class];
    }
