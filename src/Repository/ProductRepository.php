...
class ProductRepository extends ServiceEntityRepository
{
     ...
     * @return Product[]
     */
    public function filter(SearchParams $params, $lang = 'fr'): PaginationInterface
    {
     ...
     * @return Product[]
     */
    public function enQSearch(string $q, int $limit = 4)
    {
        /** @var Product[] */
        $products = $this->createQueryBuilder('p')
                    ->select('p', 'c', 'sc')
                    ...
                    ->leftJoin('p.subCategory', 'sc')
                    ->where('p.stock > 0')
                    ->andWhere('(p.enDesignation LIKE :q OR c.enName LIKE :q OR sc.enName LIKE :q)')
                    ->setParameter('q', '%'.$q.'%')
                    ->setMaxResults($limit)
                    ->getQuery()
                    ->getResult()
    ...
    public function enCountQSearch(string $q): int 
    {
        return $this->createQueryBuilder('p')
                    ->select('COUNT(p.id) as count')
                    ->leftJoin('p.category', 'c')
    ...
    private function applyFilters(QueryBuilder $qb, SearchParams $params, $lang = 'fr'): void
    {
        ...
        if($params->getQ() !== null && $params->getQ() !== '')
        {
            ...
            {
                $qb->andWhere('(p.enDesignation LIKE :q OR c.enName LIKE :q OR sc.enName LIKE :q)')
                    ->setParameter('q', '%'.$params->getQ().'%')
                    ;
            }
        }
        if(count($params->getMaterial()) > 0)
        {
            $queries = [];
            $i = 0;
            foreach($params->getMaterial() as $material)
            {
                if(in_array($material, SiteConfig::PRODUCT_MATERIALS))
                {
                    $i++;
                    $queries[] = 'p.material = :material_'.$i;
                    $qb->setParameter('material_'.$i, $material);
                }
            }
            if(count($queries) > 0)
            {
                $qb->andWhere('('.implode(' OR ', $queries).')');
            }
        }
    }
    ...
    private function applySort(QueryBuilder $qb, SearchParams $params): void 
    {
        ...
        elseif($params->getSort() === 'createdAt_DESC')
        {
            $qb->orderBy('p.createdAt', 'DESC');
        }
        elseif($params->getSort() === 'price_ASC')
        {
            $qb->orderBy('p.price', 'ASC');
        }
        elseif($params->getSort() === 'price_DESC')
        {
