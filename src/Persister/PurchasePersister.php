...
class PurchasePersister 
{
        ...
        //basics
            $purchase
                ...
                ->setShippingInfo([
                    ...
                    'price' => $checkoutData->deliveryMethod->price,
                    'mode' => $checkoutData->deliveryMethod->mode,
                    'relay' => $checkoutData->deliveryMethod->relay ?? null
                ])
    ...
    private function createDeliveryDetail(stdClass $address): PostalDetail
    {
        return (new PostalDetail)
                ...
                ->setEnCountry($address->enCountry)
                ->setIso($address->iso)
                ->setContinents($address->continents)
                ->setCreatedAt($this->frDateTimeGenerator->generateImmutable())
                ;
    }
