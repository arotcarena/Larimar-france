...
class CartService
{
    ...
    public function add(int $productId, int $quantity = 1)
    {
        ...
        if(!$product)
        {
            throw new NotFoundResourceException('Action impossible');
        }
        ...
        {
            // ON TRAVAILLE AVEC DES PIECES UNIQUES
    ...
    private function createFullCart($productsById = null): Cart
    {
        ...
        foreach($sessionCart as $productId => $quantity)
        {
            //au cas ou un produit en sessionCart a été supprimé par l'admin et updateCart n'a pas été fait (updateCart est fait seulement pour getFullCart, pas pour lightCart)
