<?php
namespace App\Controller\En\Api\Contact;
...
class ApiContactController extends AbstractController
{
    ...
    #[Route('/en/api/contact/fingerSize', name: 'en_api_contact_fingerSize')]
        ...
        try 
        {
            ...
            if(count($errors) === 0)
            {
                $this->adminNotificationEmail->send(
                    'Demande de taille "'.$fingerSizeContact->getFingerSize().'" pour le produit "'.$product->getDesignation().' (Réf. publique: '.$product->getPublicRef().')". Répondre à '.$fingerSizeContact->getEmail().' (a consulté la version anglaise du site)'
                );
