...
if(document.getElementById('suggested-products-input')) {
    ...
    const error = suggestedProductInput.querySelector('.admin-form-error ul li:first-child')?.innerHTML ?? null;
    const suggestedProductInputRoot = createRoot(suggestedProductInput);

    suggestedProductInputRoot.render(
        <SuggestedProductsInput productId={suggestedProductInput.dataset?.productid} formError={error} />
    );
}
