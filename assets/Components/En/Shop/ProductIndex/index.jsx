...
export const ProductIndex = ({categoryId, subCategoryId}) => {
    ...
    const [result, loading, error] = useFetchWithDelay('/en/api/product/index', filters, 'GET', 300);
            ...
            </div>
            {
                result && (
                    <div className={'product-index-results' + (loading ? ' loading': '')}>
                        ...
                        <p className="product-index-count">{result.count} result{result.count > 1 ? 's': ''}</p>
