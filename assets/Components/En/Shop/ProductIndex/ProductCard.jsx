...
export const ProductCard = ({product}) => {
    ...
    const handleCartAdd = async e => {
        e.preventDefault();
        try {
            await addFetch('/en/api/cart/add/id-'+product.id+'_quantity-'+quantityToAdd);
    ...
    return (
        <li className="product-card" onMouseOver={openCartButton} onMouseLeave={closeCartButton}>
            ...
            <div className="product-card-body">
                ...
                {
                    product.material && (
                        <p className="product-card-text muted">{EnTextConfig.PRODUCT_MATERIALS[product.material]}</p>
                    )
                }
            ...
            <div className="product-card-footer">
                <FormButton lang="en" loading={loading} onClick={handleCartAdd} additionalClass={'product-card-button' + (cartButtonIsOpen || isTouching ? ' open': '')}>Add to cart</FormButton>
            </div>
            {
