...
export const PaymentForm = ({piSecret, checkoutData, cart}) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await apiFetch('/fr/api/purchase/lastVerificationBeforePayment', {
        method: 'POST',
        body: JSON.stringify({
          piSecret: piSecret,
      ...
      if(e instanceof ApiError) {
        ...
        setErrors(['Le formulaire est invalide']);
      }
      setIsLoading(false);
      return;
    }
    ...
    const { error } = await stripe.confirmPayment({
      ...
      confirmParams: {
        ...
        return_url: 'https://localhost:8000/fr/commande-validee'
      },
    });
    ...
    <form id="payment-form" onSubmit={handleSubmit}>
      ...
      <div className={'checkbox-group' + (agreeCGVError ? ' is-invalid': '')}>
          ...
          <label htmlFor="checkboxRememberMe" className="form-label">
              ...
              <span id="checkbox-label">J'accepte les conditions générales de vente</span>
          </label>
          {
            agreeCGVError && <div className="form-error">Vous devez cocher cette case</div>
          }
      </div>
        ...
        <span id="button-text">
          {
            ...
            <>
              <Loader />
              <span className="no-opacity">Paiement en cours... Veuillez rester sur cette page</span>
            </>
            :
            <span>
                Payer {cart ? priceFormater(cart.totalPrice + checkoutData.deliveryMethod.price): ''}
            </span>
          }
        </span>
      </button>
