...
export const Payment = ({checkoutData, cart}) => {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                ...
                const data = await apiFetch('/fr/api/payment/createPaymentIntent', {
                    method: 'POST',
                    body: JSON.stringify({
                        totalPrice: totalPrice,
                        shippingCost: checkoutData.deliveryMethod.price
                    }) // si on envoie totalPrice undefined c'est le server qui se chargera de récupérer le cart
                });
    ...
    return (
        <div className="form-block">
            <h3 className="form-block-title">Réglement</h3>
            {clientSecret && (
                <Elements options={options} stripe={stripePromise}>
                    <PaymentForm piSecret={clientSecret} checkoutData={checkoutData} cart={cart} />
                </Elements>
            )}
        </div>
    )
